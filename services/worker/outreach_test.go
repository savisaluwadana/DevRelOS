...
func TestBuildSMTPMessageSanitizesHeaders(t *testing.T) {
	...
	text := string(message)
	if !strings.HasPrefix(messageID, "<11111111111111111111111111111111.") { t.Fatalf("unexpected message id: %s", messageID) }
	if strings.Contains(text, "\r\nBcc: attacker@example.com\r\n") { t.Fatalf("header injection survived: %s", text) }
	if !strings.Contains(text, "line one\r\nline two") { t.Fatalf("body was not normalized to CRLF: %q", text) }
	if !strings.Contains(text, "Message-ID: "+messageID) { t.Fatal("message id header missing") }
}
...
func TestLoadSMTPConfigRejectsAuthWithoutPassword(t *testing.T) {
	...
	if _, err := loadSMTPConfig(); err == nil { t.Fatal("expected missing password error") }
}
