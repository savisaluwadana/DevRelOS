...
func (a *api) createOutreach(w http.ResponseWriter, r *http.Request) {
	...
	if strings.TrimSpace(input.Channel) == "" || strings.TrimSpace(input.Body) == "" {
...
func (a *api) updateOutreachStatus(w http.ResponseWriter, r *http.Request) {
	...
	current, err := a.store.OutreachStatus(r.Context(), projectID, r.PathValue("id"))
	if err != nil {
		writeError(w, err)
		return
	}
	if !validOutreachTransition(current, input.Status) {
