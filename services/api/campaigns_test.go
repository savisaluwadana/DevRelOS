...
func TestCampaignStatusValidation(t *testing.T) {
	...
	if validCampaignStatus("sent") {
		t.Fatal("unexpected campaign status accepted")
	}
}
...
func TestCampaignStatusTransitions(t *testing.T) {
	...
	for _, pair := range allowed {
		if !campaignStatusTransitionAllowed(pair[0], pair[1]) {
			t.Fatalf("expected %s -> %s", pair[0], pair[1])
		}
	}
	if campaignStatusTransitionAllowed("archived", "active") {
		t.Fatal("archived campaign must remain terminal")
	}
	if campaignStatusTransitionAllowed("planning", "completed") {
		t.Fatal("planning campaign should activate before completion")
	}
}
