...
func (a *api) updateMediaTranscript(w http.ResponseWriter, r *http.Request) {
	...
	if err := decodeJSON(r, &input); err != nil { writeBadRequest(w, err.Error()); return }
	if strings.TrimSpace(input.Text) == "" && len(input.Segments) == 0 { writeBadRequest(w, "transcript text or segments are required"); return }
	for _, segment := range input.Segments {
		if segment.StartMS < 0 || segment.EndMS <= segment.StartMS || strings.TrimSpace(segment.Text) == "" { writeBadRequest(w, "invalid transcript segment"); return }
	}
	projectID, err := a.projectID(r); if err != nil { writeError(w, err); return }
...
func (a *api) createMediaClip(w http.ResponseWriter, r *http.Request) {
	...
	if err := decodeJSON(r, &input); err != nil { writeBadRequest(w, err.Error()); return }
	if input.MediaAssetID == "" || strings.TrimSpace(input.Title) == "" { writeBadRequest(w, "mediaAssetId and title are required"); return }
...
func (a *api) updateMediaClipStatus(w http.ResponseWriter, r *http.Request) {
	...
	if !allowed[input.Status] { writeBadRequest(w, "status must be candidate, approved or rejected"); return }
	projectID, err := a.projectID(r); if err != nil { writeError(w, err); return }
	if err := a.store.UpdateMediaClipStatus(r.Context(), projectID, r.PathValue("id"), input.Status); err != nil { writeError(w, err); return }
...
func (a *api) queueMediaRender(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { writeError(w, err); return }
	approved := false
	for _, clip := range clips { if clip.ID == r.PathValue("id") && clip.Status == "approved" { approved = true; break } }
	if !approved { writeJSON(w, http.StatusConflict, map[string]string{"error":"clip must be approved before rendering"}); return }
