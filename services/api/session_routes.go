...
func sessionLifetime() time.Duration {
	...
	if raw := strings.TrimSpace(os.Getenv("DEVRELOS_SESSION_MAX_AGE_SECONDS")); raw != "" {
		if parsed, err := strconv.Atoi(raw); err == nil { seconds = parsed }
	}
...
func (a *api) listMyWorkspaces(w http.ResponseWriter, r *http.Request) {
	act := currentActor(r)
	if act.Kind != "user" { writeForbidden(w); return }
...
func (a *api) createBrowserSession(w http.ResponseWriter, r *http.Request) {
	act := currentActor(r)
	if act.Kind != "user" { writeForbidden(w); return }
	var input struct { WorkspaceID string `json:"workspaceId"` }
	...
	if input.WorkspaceID == "" {
		if act.WorkspaceID != "" {
			...
			items, err := a.store.ListUserWorkspaces(r.Context(), act.UserID)
			if err != nil { writeError(w, err); return }
			if len(items) == 0 { writeForbidden(w); return }
			input.WorkspaceID = items[0].WorkspaceID
		}
	}
	if _, err := a.store.RoleForWorkspace(r.Context(), act.UserID, input.WorkspaceID); err != nil { writeForbidden(w); return }
...
func (a *api) switchBrowserSessionWorkspace(w http.ResponseWriter, r *http.Request) {
	act := currentActor(r)
	if act.Kind != "user" || act.SessionID == "" { writeBadRequest(w, "dedicated browser session required"); return }
	var input struct { WorkspaceID string `json:"workspaceId"` }
	if err := decodeJSON(r, &input); err != nil { writeBadRequest(w, err.Error()); return }
	input.WorkspaceID = strings.TrimSpace(input.WorkspaceID)
	if input.WorkspaceID == "" { writeBadRequest(w, "workspaceId is required"); return }
	if err := a.store.SwitchSessionWorkspace(r.Context(), act.SessionID, act.UserID, input.WorkspaceID); err != nil { writeForbidden(w); return }
	_ = a.store.AppendAuditEvent(r.Context(), identitydomain.AuditEvent{
		WorkspaceID: input.WorkspaceID, ActorUserID: act.UserID, ActorKind: "user",
...
func (a *api) revokeBrowserSession(w http.ResponseWriter, r *http.Request) {
	act := currentActor(r)
	if act.Kind != "user" || act.SessionID == "" { writeBadRequest(w, "dedicated browser session required"); return }
	if err := a.store.RevokeSession(r.Context(), act.SessionID, act.UserID); err != nil { writeError(w, err); return }
	_ = a.store.AppendAuditEvent(r.Context(), identitydomain.AuditEvent{
...
func (a *api) createInvitation(w http.ResponseWriter, r *http.Request) {
	...
	if act.Kind == "user" && input.Role == "owner" {
		...
		if roleErr != nil || role != "owner" { writeForbidden(w); return }
	}
...
func (a *api) revokeInvitation(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { writeError(w, err); return }
	if !a.requireWorkspaceRole(w, r, workspaceID, "admin") { return }
	if err := a.store.RevokeInvitation(r.Context(), workspaceID, r.PathValue("id")); err != nil { writeError(w, err); return }
	act := currentActor(r)
	_ = a.store.AppendAuditEvent(r.Context(), identitydomain.AuditEvent{
		WorkspaceID: workspaceID, ActorUserID: act.UserID, ActorKind: auditActorKind(act),
...
func (a *api) acceptInvitation(w http.ResponseWriter, r *http.Request) {
	...
	if !strings.HasPrefix(input.Token, "di_") { writeBadRequest(w, "valid invitation token is required"); return }
