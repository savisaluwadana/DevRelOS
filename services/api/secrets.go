...
func (a *api) rotateSecret(w http.ResponseWriter, r *http.Request) {
	...
	if err := decodeJSON(r, &input); err != nil { writeBadRequest(w, err.Error()); return }
	input.Value = strings.TrimSpace(input.Value)
	if input.Value == "" { writeBadRequest(w, "value is required"); return }
...
func (a *api) updateConnectorSecret(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { writeError(w, err); return }
	if !a.requireWorkspaceRole(w, r, workspaceID, "admin") { return }
	var input struct { SecretID string `json:"secretId"` }
	if err := decodeJSON(r, &input); err != nil { writeBadRequest(w, err.Error()); return }
	input.SecretID = strings.TrimSpace(input.SecretID)
	if input.SecretID != "" {
		connectors, listErr := a.store.ListConnectors(r.Context(), workspaceID)
		if listErr != nil { writeError(w, listErr); return }
		provider := ""
		for _, connector := range connectors {
			if connector.ID == r.PathValue("id") { provider = connector.Provider; break }
		}
		if provider == "" { writeJSON(w, http.StatusNotFound, map[string]string{"error": "connector not found"}); return }
	...
	if err := a.store.UpdateConnectorSecret(r.Context(), workspaceID, r.PathValue("id"), input.SecretID); err != nil { writeError(w, err); return }
	act := currentActor(r)
	_ = a.store.AppendAuditEvent(r.Context(), identitydomain.AuditEvent{
		WorkspaceID: workspaceID, ActorUserID: act.UserID, ActorKind: auditActorKind(act),
