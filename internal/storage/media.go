...
func (s *Store) UpdateMediaTranscript(ctx context.Context, projectID, id string, update mediadomain.TranscriptUpdate) (mediadomain.Asset, error) {
	...
	if err != nil { return mediadomain.Asset{}, err }
	row := s.pool.QueryRow(ctx, `
...
func (s *Store) ListMediaClips(ctx context.Context, projectID, mediaAssetID string) ([]mediadomain.Clip, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID,&item.ProjectID,&item.MediaAssetID,&item.ContentAssetID,&item.Title,&item.StartMS,&item.EndMS,
			&item.AspectRatio,&item.Score,&item.Rationale,&item.CaptionText,&item.Status,&item.OutputPath,&metadata,&item.CreatedAt,&item.UpdatedAt); err != nil { return nil, err }
		item.Metadata = map[string]any{}; _ = json.Unmarshal(metadata, &item.Metadata)
		items = append(items, item)
	}
	return items, rows.Err()
}
...
func (s *Store) UpdateMediaClipStatus(ctx context.Context, projectID,id,status string) error {
	...
	if err != nil { return err }; if cmd.RowsAffected()==0 { return pgx.ErrNoRows }; return nil
}
...
func (s *Store) ClaimNextMediaJob(ctx context.Context) (*mediadomain.Job, error) {
	...
	if err == pgx.ErrNoRows { return nil, tx.Commit(ctx) }
	if err != nil { return nil, err }
	job.Payload=map[string]any{}; _=json.Unmarshal(payload,&job.Payload)
	now:=time.Now().UTC(); job.StartedAt=&now; job.Status="running"; job.Attempts++
	if _,err=tx.Exec(ctx,`UPDATE media_jobs SET status='running',attempts=attempts+1,started_at=$2,updated_at=now() WHERE id=$1`,job.ID,now);err!=nil{return nil,err}
	if err=tx.Commit(ctx);err!=nil{return nil,err}; return &job,nil
}

func (s *Store) FinishMediaJob(ctx context.Context, job mediadomain.Job, outputPath string) error {
	now:=time.Now().UTC(); job.FinishedAt=&now
	tx,err:=s.pool.Begin(ctx); if err!=nil{return err}; defer tx.Rollback(ctx)
	if _,err=tx.Exec(ctx,`UPDATE media_jobs SET status=$2,error=$3,finished_at=$4,updated_at=now() WHERE id=$1`,job.ID,job.Status,job.Error,now);err!=nil{return err}
	if job.ClipID!="" {
		clipStatus:="failed"; if job.Status=="succeeded" { clipStatus="rendered" }
		if _,err=tx.Exec(ctx,`UPDATE media_clips SET status=$2,output_path=CASE WHEN $3<>'' THEN $3 ELSE output_path END,updated_at=now() WHERE id=$1`,job.ClipID,clipStatus,outputPath);err!=nil{return err}
	}
...
func scanMediaAsset(scan mediaAssetScanner) (mediadomain.Asset,error) {
	var item mediadomain.Asset; var segments,metadata []byte
	...
	if err!=nil{return item,err}; item.TranscriptSegments=[]mediadomain.Segment{}; item.Metadata=map[string]any{}
	_ = json.Unmarshal(segments,&item.TranscriptSegments); _=json.Unmarshal(metadata,&item.Metadata); return item,nil
}
