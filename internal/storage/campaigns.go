...
func (s *Store) ListCampaigns(ctx context.Context, projectID string) ([]campaigndomain.Campaign, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.ProjectID, &item.Name, &item.Objective, &item.Status, &item.StartsAt, &item.EndsAt,
			&item.BudgetUSD, &target, &item.CreatedAt, &item.UpdatedAt); err != nil { return nil, err }
...
func (s *Store) CreateCampaign(ctx context.Context, item campaigndomain.Campaign) (campaigndomain.Campaign, error) {
	...
	if item.Target == nil { item.Target = map[string]any{} }
...
func (s *Store) CampaignEntityBelongsToProject(ctx context.Context, projectID, entityType, entityID string) (bool, error) {
	...
	if !ok { return false, fmt.Errorf("unsupported campaign entity type %q", entityType) }
	var exists bool
...
func (s *Store) RecordCampaignMetric(ctx context.Context, projectID string, metric campaigndomain.Metric) (campaigndomain.Metric, error) {
	...
	if metric.ObservedAt.IsZero() { metric.ObservedAt = time.Now().UTC() }
	if metric.Metadata == nil { metric.Metadata = map[string]any{} }
	metadata, err := json.Marshal(metric.Metadata)
	if err != nil { return metric, err }
	err = s.pool.QueryRow(ctx, `
...
func (s *Store) CampaignReport(ctx context.Context, projectID, campaignID string) (campaigndomain.Report, error) {
	...
	if err != nil { return campaigndomain.Report{}, err }
	...
	for rows.Next() {
		...
		var count int
		var cost float64
		if err := rows.Scan(&kind, &count, &cost); err != nil { rows.Close(); return report, err }
	...
	if err != nil { return report, err }
	if report.OutreachSent > 0 { report.ReplyRate = float64(report.OutreachReplies) / float64(report.OutreachSent) * 100 }
	if report.Submissions > 0 { report.AcceptanceRate = float64(report.AcceptedTalks) / float64(report.Submissions) * 100 }
	...
	for metricRows.Next() {
		...
		var metadata []byte
		if err := metricRows.Scan(&m.ID, &m.CampaignID, &m.MetricKey, &m.MetricValue, &m.Source, &m.ObservedAt, &metadata, &m.CreatedAt); err != nil { metricRows.Close(); return report, err }
		m.Metadata = map[string]any{}
		_ = json.Unmarshal(metadata, &m.Metadata)
		report.Metrics[m.MetricKey] += m.MetricValue
		if len(report.RecentMetrics) < 20 { report.RecentMetrics = append(report.RecentMetrics, m) }
	}
	if err := metricRows.Err(); err != nil { metricRows.Close(); return report, err }
	metricRows.Close()
...
func (s *Store) RelationshipRadar(ctx context.Context, projectID string) ([]campaigndomain.RelationshipRadarItem, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.RelationshipID, &item.ProjectID, &item.CommunityID, &item.ContactID, &item.Name, &item.Kind,
			&item.Stage, &item.Strength, &item.LastTouchAt, &item.NextFollowUpAt); err != nil { return nil, err }
		risk := (100 - item.Strength) / 2
		if item.LastTouchAt == nil { risk += 25 } else {
			days := int(now.Sub(*item.LastTouchAt).Hours() / 24)
			if days < 0 { days = 0 }
			item.DaysSinceTouch = &days
			if days > 60 { risk += 30 } else if days > 30 { risk += 20 } else if days > 14 { risk += 10 }
		}
		if item.NextFollowUpAt != nil && item.NextFollowUpAt.Before(now) { risk += 30 }
		if item.Stage == "dormant" { risk += 20 }
