...
func (s *Store) ListUserWorkspaces(ctx context.Context, userID string) ([]domain.WorkspaceAccess, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.WorkspaceID, &item.WorkspaceSlug, &item.WorkspaceName, &item.Role); err != nil { return nil, err }
...
func (s *Store) ListInvitations(ctx context.Context, workspaceID string) ([]domain.Invitation, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.WorkspaceID, &item.Email, &item.Role, &item.InvitedByUserID, &item.ExpiresAt, &item.AcceptedAt, &item.CreatedAt); err != nil { return nil, err }
		items = append(items, item)
	}
	return items, rows.Err()
}
...
func (s *Store) CreateInvitation(ctx context.Context, item domain.Invitation, tokenHash string) (domain.Invitation, error) {
	...
	if err != nil { return item, err }
	if err := tx.Commit(ctx); err != nil { return item, err }
	return item, nil
}
...
func (s *Store) AcceptInvitation(ctx context.Context, tokenHash, displayName string) (domain.User, domain.Invitation, error) {
	tx, err := s.pool.Begin(ctx)
	if err != nil { return domain.User{}, domain.Invitation{}, err }
	defer func() { _ = tx.Rollback(ctx) }()
	...
	if err != nil { return domain.User{}, invitation, err }
	...
	if err != nil { return user, invitation, err }
	if user.Status != "active" { return user, invitation, errors.New("invited user is disabled") }
	...
	if err := tx.Commit(ctx); err != nil { return user, invitation, err }
