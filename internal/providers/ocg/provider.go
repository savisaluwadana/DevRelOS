...
func (p *Provider) Fetch(ctx context.Context, config map[string]any, request connectors.FetchRequest) (connectors.FetchResult, error) {
	if err := p.ValidateConfig(config); err != nil { return connectors.FetchResult{}, err }
	base := defaultBaseURL
	if value, ok := config["base_url"].(string); ok && value != "" { base = strings.TrimRight(value, "/") }
	...
	offset := 0
	if request.Cursor != "" {
		if parsed, parseErr := strconv.Atoi(request.Cursor); parseErr == nil && parsed >= 0 { offset = parsed }
	}

	q := endpoint.Query()
	community := stringValue(config, "community", "cncf")
	if community != "" { q.Set("community[0]", community) }
	if query := strings.TrimSpace(stringValue(config, "query", "")); query != "" { q.Set("ts_query", query) }
	if region := strings.TrimSpace(stringValue(config, "region", "")); region != "" { q.Set("region[0]", region) }
	if category := strings.TrimSpace(stringValue(config, "group_category", "")); category != "" { q.Set("group_category[0]", category) }
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 { return connectors.FetchResult{}, fmt.Errorf("ocg returned %s", resp.Status) }

	var payload searchResponse
	if err := json.NewDecoder(resp.Body).Decode(&payload); err != nil { return connectors.FetchResult{}, err }
	result := connectors.FetchResult{Records: make([]connectors.RawRecord, 0, len(payload.Groups)), RequestsMade: 1}
	if offset+len(payload.Groups) < payload.Total { result.NextCursor = strconv.Itoa(offset + len(payload.Groups)) }

	for _, group := range payload.Groups {
		if !group.Active || group.GroupID == "" || group.Name == "" { continue }
		slug := group.Slug
		if group.SlugPretty != nil && *group.SlugPretty != "" { slug = *group.SlugPretty }
		canonical := ""
		if group.CommunityName != "" && slug != "" {
			canonical = base + "/" + url.PathEscape(group.CommunityName) + "/group/" + url.PathEscape(slug)
		}
		var timestamp *time.Time
		if group.CreatedAt > 0 { value := time.Unix(group.CreatedAt, 0).UTC(); timestamp = &value }
...
func stringValue(config map[string]any, key, fallback string) string {
	if value, ok := config[key].(string); ok { return value }
	return fallback
}
func ptrValue(value *string) string { if value == nil { return "" }; return *value }
func topicsFromConfig(config map[string]any) []string {
	value, ok := config["topics"]
	if !ok { return nil }
	switch typed := value.(type) {
	case string: return strings.Split(typed, ",")
	case []string: return typed
	case []any:
		out := make([]string, 0, len(typed)); for _, item := range typed { if text, ok := item.(string); ok { out = append(out, text) } }; return out
	default: return nil
	}
}
func categoryLabels(raw json.RawMessage) []string {
	...
	var text string
	if json.Unmarshal(raw, &text) == nil && text != "" { return []string{text} }
	var object map[string]any
	if json.Unmarshal(raw, &object) == nil {
		for _, key := range []string{"name", "slug", "normalized_name"} { if value, ok := object[key].(string); ok && value != "" { return []string{value} } }
	}
	return nil
}
func unique(items []string) []string {
	seen := map[string]bool{}; out := make([]string, 0, len(items))
	for _, item := range items { item = strings.TrimSpace(strings.ToLower(item)); item = strings.ReplaceAll(item, " ", "-"); if item != "" && !seen[item] { seen[item] = true; out = append(out, item) } }
	return out
}
